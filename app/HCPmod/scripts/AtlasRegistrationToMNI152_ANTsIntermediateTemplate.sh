...
echo " "
echo ${ANTSPATH}${ANTSPATH:+/}antsRegistrationSyN.sh -d 3 -f ${StudyTemplate} -m ${T1wRestore}.nii.gz -o ${WD}/xfms/${T1wRestoreBasename}2${StudyTemplateBasename}_
${ANTSPATH}${ANTSPATH:+/}antsRegistrationSyN.sh -d 3 -f ${StudyTemplate} -m ${T1wRestore}.nii.gz -o ${WD}/xfms/${T1wRestoreBasename}2${StudyTemplateBasename}_

echo " ANTs T1w2StudyTemplate Registration to MNI"
echo " "

# Register The T1w2StudyTemplate to MNI atlas
${ANTSPATH}${ANTSPATH:+/}antsRegistrationSyN.sh -d 3 -j 1 -f ${Reference} -m ${WD}/xfms/${T1wRestoreBasename}2${StudyTemplateBasename}_Warped.nii.gz -x ${ReferenceMask},${T1wMask}.nii.gz -o ${WD}/xfms/T1w${StudyTemplateBasename}_to_MNI_

echo " antsApplyTransform"
echo " "
# combine all the affine and non-linear warps in the order: W2, A2, W1, A1
${ANTSPATH}${ANTSPATH:+/}antsApplyTransforms -d 3 -i ${T1wRestore}.nii.gz -r ${Reference} -t ${WD}/xfms/T1w${StudyTemplateBasename}_to_MNI_1Warp.nii.gz -t ${WD}/xfms/T1w${StudyTemplateBasename}_to_MNI_0GenericAffine.mat -t ${WD}/xfms/${T1wRestoreBasename}2${StudyTemplateBasename}_1Warp.nii.gz -t ${WD}/xfms/${T1wRestoreBasename}2${StudyTemplateBasename}_0GenericAffine.mat -o [${WD}/xfms/ANTs_CombinedWarp.nii.gz,1] 

# combine inverse warps in the order A1, W1, A2, W2
${ANTSPATH}${ANTSPATH:+/}antsApplyTransforms -d 3 -i ${T1wImage}.nii.gz -r ${Reference} -t [${WD}/xfms/${T1wRestoreBasename}2${StudyTemplateBasename}_0GenericAffine.mat,1] -t ${WD}/xfms/${T1wRestoreBasename}2${StudyTemplateBasename}_1InverseWarp.nii.gz -t [${WD}/xfms/T1w${StudyTemplateBasename}_to_MNI_0GenericAffine.mat,1] -t ${WD}/xfms/T1w${StudyTemplateBasename}_to_MNI_1InverseWarp.nii.gz -o [${WD}/xfms/ANTs_CombinedInvWarp.nii.gz,1] 
